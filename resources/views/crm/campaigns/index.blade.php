...
<div class="flex flex-col gap-6" x-data="campaignManager()">
    <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        <div>
            ...
            <p class="text-slate-500 mt-1">Kelola strategi koleksi per bucket & tipe</p>
        </div>
        <button 
...
function campaignManager() {
    return {
        ...
        async submitForm() {
            ...
            const url = isEdit ? `{{ route('crm.campaigns.index') }}/${this.form.id}` : '{{ route('crm.campaigns.index') }}';
