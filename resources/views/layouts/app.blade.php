    ...
    <aside class="w-64 bg-slate-900 text-slate-300 flex flex-col h-full shadow-xl hidden md:flex justify-between select-none">
        <div>
            ...
            <nav class="p-4 space-y-6 overflow-y-auto">
                ...
                <!-- ADMIN AREA (Jika Admin, letakkan di atas agar mudah diakses) -->
                @if(auth()->check() && auth()->user()->role === 'admin')
                <div>
                    <p class="px-3 text-[10px] font-bold text-slate-500 uppercase tracking-wider mb-2">Admin Management</p>
                    <div class="space-y-1">
                        <!-- Agent Management -->
                ...
                <!-- MONITORING & SUPERVISOR AREA -->
                <div>
                    <p class="px-3 text-[10px] font-bold text-slate-500 uppercase tracking-wider mb-2">Monitoring & Reports</p>
                    <div class="space-y-1">
                        <!-- Live Monitoring -->
        ...
        <!-- User Profile di Bawah Sidebar (Dibuat Dinamis & Aman untuk Logout) -->
        <div class="p-4 bg-slate-950 border-t border-slate-800 flex items-center justify-between">
            ...
            <!-- Tombol Logout yang otomatis menyesuaikan (Admin / Supervisor) -->
            <form action="{{ session()->has('supervisor_extension') ? route('supervisor.logout') : route('logout') }}" method="POST">
                @csrf
